...
void MS::AddThroughput(Real prob)
{
	...
	SCHEDULE_DECISION my_sector_decision = pppSchedule_Map[my_sec_idx][0][0];
	...
	else
	{
		...
		if (links[self_idx].num_re_tx > 30)
		{
			links[self_idx].num_re_tx = 0;
			links[self_idx].ACK = true;
...
Real MS::Return_initial_packet_error(void)
{
...
Real MS::compute_tone_SINR(int rb_idx)
{
	...
	const Real linear_signal = dBm2linear(bs_maxpower - channel[links[self_idx]._sector_in_control][self_idx].pathloss_final);
