	...
	else if (Scheduling_Type == 1)
	{
		...
		#pragma omp parallel num_threads(num_of_threads)
		{
			...
			for (int sec_idx = start_sector; sec_idx < end_sector; sec_idx++)
			{
				...
				//sector[sec_idx].Scheduling_algorithm_module_MU_MIMO();
			}
		#if ENABLE_MULTITHREADING
		}
		#endif
...
void Sector::Read_Ch_Feedback(void)
{
	...
	if (PMI_read == NULL)
	{
		PMI_read         = new int          * [fixed_mem];  //[ue_in_control.size() + ue_in_comp.size()]; //30
		...
		{
			PMI_read[ue_idx]         = new       int [num_rb];
...
void Sector::Set_AVR_Cqi_Precoding_Based(void)
{
	...
	for (int ue_idx = 0; ue_idx < (int)ue_in_control.size(); ue_idx++)
	{
		...
		const Real linear_signal = dBm2linear(bs_maxpower - channel[self_idx][ms_idx].pathloss_final);
		...
		if (count_subbands > 0)
		{
			...
			const Real avr_sinr_dB = 10.0 * log10(selected_sinr_linear);

			// Determine MCS and CQI based on selected SINR
...
void Sector::Scheduling_algorithm_module_MU_MIMO(void)
{
	...
	for (int subband_idx = 0; subband_idx < num_subband; subband_idx++)
	{
		...
		// Perform user selection for this subband
